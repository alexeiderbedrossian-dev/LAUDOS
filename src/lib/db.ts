...
export function ensureDbReady(): Promise<void> {
  ...
  return getSql()
    .then(() => seedClinicLogin())
    .then(() => undefined);
}

async function seedClinicLogin() {
  const g = globalThis as typeof globalThis & { __clinicLoginSeeded__?: boolean };
  if (g.__clinicLoginSeeded__) return;
  g.__clinicLoginSeeded__ = true;
  try {
    const { auth } = await import("@/lib/auth/server");
    await auth.api.signUpEmail({
      body: {
        name: "Alexei",
        email: "alexeicuritiba@me.com",
        password: "Merces2026",
      },
    });
  } catch {
